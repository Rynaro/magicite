...
intent:
  ...
  use_when: "the Magicite container exits or hangs at startup, or throws PermissionError against .magicite, under docker run"
...
triggers:
  ...
  - "permissionerror on .magicite when running magicite in docker"
...
- (x1) Running the container as root to sidestep it. The files it then creates in `.magicite` are root-owned on the host, which breaks the next non-root run and every later local `magicite sync`.
...
## Examples
+ "magicite in docker throws PermissionError creating .magicite/runtime" -> steps 2 and 3
