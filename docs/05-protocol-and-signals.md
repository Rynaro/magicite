...
| **1** | Tool-mediated self-report: `signal_use` + `signal_outcome` called by agent following instructions | Any host; probabilistic compliance (60–80%) | `self_reported` | Medium (70–85%) | Rate-capped per skill per session; conversion to durable state is time-gated; can update S via Dream at 0.6 weight |
